...
$(document).ready(function () {
    ...
    document.onkeydown = function (e) {
        e = e || window.keyCode;
        if ($('.header-search-list').is(':visible')) {
            if (e.keyCode == 38) {
                if ($('.header-search-list li.active').length > 0 && $('.header-search-list li.active').index() != 0) {
                    var myLi = $('.header-search-list li').eq($('.header-search-list li.active').index() - 1);
                    setActive(myLi);
                }
                else {
                    var myLi = $('.header-search-list li').eq(0);
                    setActive(myLi);
                }
            }
            if (e.keyCode == 40) {
                if ($('.header-search-list li.active').length == 0) {
                    var myLi = $('.header-search-list li').eq(0);
                    setActive(myLi);
                }
                else if ($('.header-search-list li.active').length > 0 && $('.header-search-list li.active').index() != $('.header-search-list li').length - 1) {
                    var myLi = $('.header-search-list li').eq($('.header-search-list li.active').index() + 1);
                    setActive(myLi);
                }
                else {
                    var myLi = $('.header-search-list li').eq($('.header-search-list li').length - 1);
                    setActive(myLi);
                }
            }
            if (e.keyCode == 13) {
                if ($('.header-search-list li.active').length > 0) {
                    $('.header-search-list li.active').click();
                }
            }
        }
    }
